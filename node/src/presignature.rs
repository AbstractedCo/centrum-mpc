use super::{
    ...
    triple::{Triple, TripleConfig, TripleId, TripleManager},
    types::{PresignatureProtocol, PublicKey, SecretKeyShare},
};
use cait_sith::{
...
impl PresignatureGenerator {
    ...
    pub fn poke(&mut self) -> Result<Action<PresignOutput<Secp256k1>>, ProtocolError> {
        if self.timestamp.elapsed() > super::types::PROTOCOL_PRESIG_TIMEOUT {
            tracing::info!(
                self.triple0,
                self.triple1,
                self.mine,
            ...
            return Err(ProtocolError::Other(
                "presignature protocol timed out".into(),
            ));
        }
...
pub struct PresignatureManager<AccountId> {
...
impl<AccountId: PartialEq + Clone + std::fmt::Debug> PresignatureManager<AccountId> {
    pub fn new(
        me: Participant,
        threshold: usize,
        epoch: u64,
    ...
    pub fn clear_taken(&mut self) {
        self.taken
            .retain(|_, instant| instant.elapsed() < super::types::TAKEN_TIMEOUT);
    }
    ...
    pub async fn stockpile(
        ...
        triple_manager: &mut TripleManager<AccountId>,
    ) -> Result<(), InitializationError> {
        ...
        if not_enough_presignatures {
            ...
            if let Some((triple0, triple1)) = triple_manager.take_two_mine().await {
                let presig_participants = active.intersection(&[
                    &triple0
                        .public
                        .participants
                        .clone()
